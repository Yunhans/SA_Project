...
public class PostHelper {
	...
	public JSONObject getAllPost() {
		...
		/** 將SQL指令與所有post資料之JSONArray，封裝成JSONObject回傳 */
	...
	public List<Integer> getAllDelPost(int member_id){
 	...
 	public JSONObject getByID(String id) {
		/** 新建一個 post 物件之 p 變數，用於紀錄每一位查詢回之post資料 */
		Post p = null;
		/** 用於儲存所有檢索回之post，以JSONArray方式儲存 */
		JSONArray jsa = new JSONArray();
		/** 記錄實際執行之SQL指令 */
		String exexcute_sql = "";
		/** 紀錄SQL總行數 */
		...
		try {
			...
			while (rs.next()) {
				...
				/** 將每一筆post資料產生一名新post物件 */
				p = new Post(page, file_path, post_id, post_title, post_type, post_description, member_id, member_name, member_account);
				/** 取出該名post之資料並封裝至 JSONsonArray 內 */
				jsa.put(p.getData());
			}
 	...
 	public JSONObject getSearch(String search_wd) {
 		System.out.println("posthelper getsearch");
		/** 新建一個 post 物件之 p 變數，用於紀錄每一位查詢回之post資料 */
		Post p = null;
		/** 用於儲存所有檢索回之post，以JSONArray方式儲存 */
		JSONArray jsa = new JSONArray();
		...
		try {
			...
			conn = DBMgr.getConnection();
			/** SQL指令 */
			String sql = "SELECT DISTINCT RANK()Over(PARTITION BY f.post_id ORDER BY f.file_id) as page, f.file_path, p.post_id,p.post_title, p.post_type, p.post_description, m.member_id, m.member_name, m.member_account "
					+ "FROM Post p "
					+ "Left JOIN file f ON p.post_id = f.post_id "
					+ "LEFT JOIN Member m ON p.member_id = m.member_id " 
		...
		/** 將所有符合查詢post資料之JSONArray，封裝成JSONObject回傳 */
		JSONObject response = new JSONObject();
		// response.put("sql", exexcute_sql);
		// response.put("row", row);
		response.put("data", jsa);
